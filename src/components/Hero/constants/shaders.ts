 ...
 * 5 Nature-Inspired Scenes with unique colors and subtle animations
 *
 * Scenes:
 * 1. Water - Wave interference ripples (Teal mist)
 * 2. Hive - Breathing hexagons (Warm amber)
 * 3. Cell - Voronoi patterns (Fresh green)
 * 4. Shell - Logarithmic spiral (Soft lavender)
 * 5. Wood - Concentric rings (Earthy taupe)
 */
...
// Scene colors for JavaScript (to sync with UI)
// Background colors are 40% less saturated (60% original + 40% white base)
export const SCENE_COLORS = {
  water: {
    bg: '#EDF7F6',    // Softened from #E0F2F1 (60% opacity blend with white)
    fg: '#80CBC4',
    accent: '#00796B',
    text: '#00413A',
    textMuted: '#00695C'  // Darkened for better contrast (WCAG AA)
  },
  hive: {
    bg: '#FDFAF2',    // Softened from #FFF8E1 (60% opacity blend with white)
    fg: '#FFE082',
    accent: '#FFC107',
    text: '#5D4037',
    textMuted: '#8D6E63'
  ...
  cell: {
    bg: '#F6FAF3',    // Softened from #F1F8E9 (60% opacity blend with white)
    fg: '#AED581',
    accent: '#558B2F',
    text: '#2E5A1C',
    textMuted: '#4E7A2D'  // Darkened for better contrast (WCAG AA)
  },
  shell: {
    bg: '#050508',    // Deep space black
    fg: '#D95F80',    // Nebula pink
    accent: '#9794F2', // Purple glow
    text: '#FFFFFF',
    textMuted: 'rgba(255, 255, 255, 0.7)',
    // Extended nebula palette for UI use
    nebula: {
      pink: '#D95F80',
      purple: '#9794F2',
      teal: '#32738C',
  ...
  wood: {
    bg: '#F5F3F2',    // Softened from #EFEBE9 (60% opacity blend with white)
    fg: '#BCAAA4',
    accent: '#795548',
    text: '#3E2723',
    textMuted: '#8D6E63'
...
export type SceneName = keyof typeof SCENE_COLORS;
export const SCENE_ORDER: SceneName[] = ['water', 'hive', 'cell', 'shell', 'wood'];

// Timing constants (must match shader)
export const SCENE_TIMING = {
  holdTime: 4.0,        // seconds per scene
  transitionTime: 1.5,  // seconds for transition
